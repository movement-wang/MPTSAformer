...
from lib.data_prepare import get_pretrain_dataloader
from model.MPformer.MPformer import MPformer
...
@torch.no_grad()
def predict(model, testset_loader, scaler):
    model.eval()
    y = []
    out = []

    for long_history_batch in testset_loader:
        long_history_batch[:, :, :, 0] = scaler.transform(long_history_batch[:, :, :, 0])
        long_history_batch = long_history_batch.to(device)

        out_batch, y_batch = model(long_history_batch)
...
def train_one_epoch(model, device, trainset_loader, scaler, optimizer, scheduler, criterion):
    ...
    for long_history_batch in trainset_loader:
        long_history_batch[..., 0] = scaler.transform(long_history_batch[..., 0])
        long_history_batch = long_history_batch.to(device)
...
@torch.no_grad()
def test_model(model, testset_loader, scaler, log):
    model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu_list", type=list, default=[0, 1])
    ...
    with open("MPformer.yaml", "r") as f:
        cfg = yaml.safe_load(f)
    cfg = cfg[dataset]

    # -------------------------------- load model -------------------------------- #

    model = MPformer(mode="pre-train", **cfg["MPformer_args"])
    ...
    log = os.path.join(log_path, f"{model_name}-{dataset}-{cfg['MPformer_args']['in_steps']}-{cfg['MPformer_args']['mask_ratio']}-{now}.log")
    ...
    trainset_loader, valset_loader, scaler = get_pretrain_dataloader(
        data_path, tod=cfg["time_of_day"], dow=cfg["day_of_week"], batch_size=cfg["batch_size"], steps=cfg['MPformer_args']["in_steps"]
    )
    ...
    save = os.path.join(save_path, f"{model_name}-{dataset}-{cfg['MPformer_args']['in_steps']}-{cfg['MPformer_args']['mask_ratio']}-{now}.pt")

    # ---------------------- set loss, optimizer, scheduler ---------------------- #

    criterion = nn.HuberLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg["lr"], weight_decay=cfg["weight_decay"])
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=cfg["milestones"], gamma=cfg["lr_decay_rate"], verbose=False)
